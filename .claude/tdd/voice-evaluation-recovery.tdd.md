...
| GREEN | `pnpm exec node --test src/pages/voiceEvaluationStatus.test.ts` | 通过 | 3 个排队、延迟、处理和失败状态用例通过。 |
| 回归 | `./gradlew :app:test --no-daemon` | 通过 | 完整后端测试套件通过。 |
| 回归 | `pnpm exec node --test ...` | 通过 | 前端现有 16 个 Node 测试全部通过。 |
...
| 9 | 历史列表显示“评估延迟 / 可重新生成”，详情页提供重新生成入口。 | 本地浏览器 + 临时测试会话 | 浏览器验收 | 通过 |
| 10 | 点击重新生成后任务进入 Redis Stream，消费者完成处理，页面自动展示报告。 | 本地 Spring Boot、Redis Stream、PostgreSQL、浏览器 | 端到端验收 | 通过 |
| 11 | 无有效对话的报告总分显示为“—”。 | 浏览器最终页面截图与 DOM | 视觉验收 | 通过 |
